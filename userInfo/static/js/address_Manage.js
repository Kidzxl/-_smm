...
var compare = function(obj1, obj2) {
    var val1 = obj1.choice;
    var val2 = obj2.choice;
    if (val1 < val2) {
        return -1;
    } else if (val1 > val2) {
        return 1;
...
var app = new Vue({
    ...
    methods:{
        getAddress:function () {
            ...
            $.ajax({
                type: "get",
                url: "http://192.168.0.106:80/ssm/userInfo/getAddress",
                data:{uid:parseInt(that.user.id)},
            ...
            this.addressList = that.addressList.sort(compare);
        },
        submit:function(){
            var that = this;
            var data = {addressSf:-1,addressCs:-1,addressQx:-1};
            $.each(cityJson,
                function(i, val) {
                    if (val.item_code == that.sf) {
                        data.addressSf = val.item_name;
                    }
                    if (val.item_code == that.cs) {
                        data.addressCs = val.item_name;
                    }
                    if (val.item_code == that.qx) {
                        data.addressQx = val.item_name;
                    }
                });
            data.name=that.name;
            data.postalCode=that.postalCode;
            data.detailAddress=that.detailAddress;
            data.tel=that.tel;
            data.choice=that.checkbox?1:0;
            data.uid = this.user.id;
            console.log(data);
            $.ajax({
                type: "POST",
                url: "http://192.168.0.106:80/ssm/userInfo/insertAddress",
                data:data,
                success: function(response){
                    console.log(response);
                    if (response.code == 200){
